...
- 2026-01-14: Multi-CSV upload showed "file already being processed" error after uploading 3 CSVs. Cause: The `reset_for_new_batch` function only cleared results but left the queue, current file tracking, and batch uploads intact, causing stale state to block new processing. Additionally, `start_next` blocked when status was "processing" even if no file was actually being processed, and `mark_error`/`mark_complete` didn't clear the current file tracking. Prevention: Fully reset all processing state (queue, current_file, batch_uploads, results, timestamp) when starting a new batch. Clear current_file in mark_error/mark_complete. Detect and recover from stale "processing" state in start_next.
