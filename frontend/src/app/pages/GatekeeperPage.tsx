...
export default function GatekeeperPage() {
  ...
  return (
    <div className="flex flex-col gap-4">
      <header>
        <h1 className="text-ui-page">Gatekeeper</h1>
        <p className="text-ui-muted">Manage the email allowlist for access.</p>
      </header>

      <form onSubmit={handleAddEmail} className="flex flex-col gap-2">
        <label className="text-ui-body font-medium" htmlFor="gatekeeper-email">
          Add email
        </label>
        <div className="flex gap-2">
          <input
            id="gatekeeper-email"
            type="email"
            className="flex-1 rounded-md border border-border bg-surface px-3 py-2 text-ui-body"
            value={pendingEmail}
          ...
          <button
            type="submit"
            className="rounded-md bg-accent px-3 py-2 text-ui-body font-medium text-white"
          >
            Add
          </button>
        </div>
      ...
      <div className="rounded-md border border-border p-4">
        <h2 className="text-ui-title">Allowed emails</h2>
        {normalizedEmails.length === 0 ? (
          <p className="mt-2 text-ui-muted">No email addresses have been added yet.</p>
        ) : (
          <ul className="mt-3 flex flex-col gap-2 text-ui-body">
            {normalizedEmails.map((email: string) => (
              <li key={email} className="flex items-center justify-between">
                <span>{email}</span>
                <button
                  type="button"
                  className="text-ui-meta font-medium text-red-400 hover:text-red-300"
