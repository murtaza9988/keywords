    ...
    <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
      <div className="flex border border-border p-0.5 gap-2 rounded bg-surface-muted w-full sm:w-auto justify-center shrink-0">
        <button
          ...
          className={`flex-1 sm:flex-none px-1 py-2 text-ui-tab rounded transition-colors hover:cursor-pointer ${activeView === 'ungrouped' ? 'bg-accent/85 text-white shadow-sm' : 'hover:text-foreground hover:bg-surface-muted'}`}
        >
          {viewLabels.ungrouped}
        </button>
        <button
          onClick={() => onViewChange('grouped')}
          className={`flex-1 sm:flex-none px-1 py-2 text-ui-tab rounded transition-colors hover:cursor-pointer ${activeView === 'grouped' ? 'bg-accent/85 text-white shadow-sm' : 'hover:text-foreground hover:bg-surface-muted'}`}
        >
          {viewLabels.grouped}
        </button>
        <button
          onClick={() => onViewChange('confirmed')}
          className={`flex-1 sm:flex-none px-1 py-2 text-ui-tab rounded transition-colors hover:cursor-pointer ${activeView === 'confirmed' ? 'bg-success/85 text-white shadow-sm' : 'hover:text-foreground hover:bg-surface-muted'}`}
        >
          {viewLabels.confirmed}
        </button>
        <button
          onClick={() => onViewChange('blocked')}
          className={`flex-1 sm:flex-none px-1 py-2 text-ui-tab rounded transition-colors hover:cursor-pointer ${activeView === 'blocked' ? 'bg-danger/85 text-white shadow-sm' : 'hover:text-foreground hover:bg-surface-muted'}`}
        >
          {viewLabels.blocked}
        </button>
      </div>
